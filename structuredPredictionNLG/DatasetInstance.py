...
def lexicalize_word_sequence(sequence, delex_map, complex_relex=False):
    if sequence and isinstance(sequence[0], Action):
        sequence = [a.label for a in sequence]

    real = []
    for act in sequence:
        if act != Action.TOKEN_GO and act != Action.TOKEN_SHIFT and not act.startswith('¬'):
            if act in delex_map:
                if real and complex_relex:
                    v = delex_map[act].split()
                    for a in v:
                        if a and real[-1] != a:
                            real.append(a)
                else:
            ...
            real = apply_non_monotonic_action(real, act)
    return real # (" ".join([o for o in real])).strip()
...
class NLGOutput:
    ...
    # it must return an evalStats object with a loss
    def evaluateAgainst(self, predicted):
        evalStats = NLGEvalStats()
